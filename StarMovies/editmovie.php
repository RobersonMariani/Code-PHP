            ...
            <div class="col-md-6 offset-md-1">
                ...
                <form id="edit-movie-form" action="<?= $BASE_URL ?>movie_process.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="length" id="length" placeholder="Digite a duração do filme" value="<?= $movie->getLength() ?>">
                    </div>
                    <div class="form-group">
                        ...
                        <select name="category" id="category" class="form-control">
                            ...
                            <?php foreach (CATEGORIES as $index => $category) : ?>
                                <option value="<?= $index ?>" <?= $movie->getCategory() === $index ? "selected" : "" ?>><?= $category ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="trailer">Trailer</label>
                        <input type="text" class="form-control" name="trailer" id="trailer" placeholder="Insira o link do trailer" value="<?= $movie->getTrailer() ?>">
                    </div>
                    <div class="form-group">
                        <label for="description">Descrição</label>
                        <textarea name="description" id="description" class="form-control" rows="5" placeholder="Descreva o filme..."><?= $movie->getDescription() ?></textarea>
                    </div>
